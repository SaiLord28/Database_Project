...
public class TelefonoEmpleadoDAO {
    ...
    public void listarPorDNI(int dni) {
        String sql = "SELECT Telefono FROM TelefonoEmpleado WHERE DNIEmpleado = ?";
        try (Connection conn = Conexion.getConexion(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, dni);
            ResultSet rs = ps.executeQuery();
            System.out.println("Teléfonos del empleado " + dni + ":");
            while (rs.next()) {
    ...
    public void eliminar(int dni, String telefono) {
        String sql = "DELETE FROM TelefonoEmpleado WHERE DNIEmpleado = ? AND Telefono = ?";
        try (Connection conn = Conexion.getConexion(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, dni);
            ps.setString(2, telefono);
